...
class AddRevisionUseCaseTest {
    ...
    @BeforeEach
    void setUp() {
        useCase = new AddRevisionUseCase(documentRepository, revisionRepository, storageService);
    }
    ...
    void addRevision_success() throws Exception {
        ...
        DocumentRevision newRevision = DocumentRevision.builder()
            ...
            .document(document)
            .revisionNumber("2.0")
            .storagePath("ged/SOP-001/uuid_sop-001-v2.pdf")
            .originalFileName("sop-001-v2.pdf")
            .fileSizeBytes(10L)
            .uploadedBy("user1")
            .uploadedAt(LocalDateTime.now())
            .changeReason("Atualização do procedimento")
            .build();
        ...
        // Act
        DocumentRevisionResponse response = useCase.execute(docId, "Atualização do procedimento", file, "user1");
        ...
        verify(storageService).upload(anyString(), any(), eq("application/pdf"), eq((long) fileContent.length));
