...
class UploadDocumentUseCaseTest {
    ...
    void createDocument_success() throws Exception {
        ...
        Document savedDocument = Document.builder()
            ...
            .category(DocumentCategory.SOP).status(DocumentStatus.DRAFT)
        ...
        DocumentRevision savedRevision = DocumentRevision.builder()
            .id(revId).document(savedDocument).revisionNumber("1.0")
            .storagePath("ged/SOP-001/uuid_sop-001.pdf").originalFileName("sop-001.pdf")
            .fileSizeBytes(8L).uploadedBy("user1").uploadedAt(LocalDateTime.now())
        ...
        when(documentRepository.save(any(Document.class)))
            .thenAnswer(inv -> {
                ...
                if (d.getId() == null) {
                    return Document.builder().id(docId).code(d.getCode()).title(d.getTitle())
                        .category(d.getCategory()).status(d.getStatus())
                        .createdBy(d.getCreatedBy()).createdAt(d.getCreatedAt())
                        .currentRevision(d.getCurrentRevision()).build();
                }
    ...
    @Test
    void createDocument_duplicateCode_throwsDocumentCodeAlreadyExistsException() {
        // SEC-129: existsByCode returns true → DocumentCodeAlreadyExistsException (409)
        CreateDocumentRequest request = new CreateDocumentRequest(
            "SOP-001", "Procedimento Operacional", DocumentCategory.SOP, "Criação inicial"
        );
        MockMultipartFile file = new MockMultipartFile(
    ...
    @Test
    void createDocument_emptyFile_throwsInvalidGedFileException() {
        // SEC-125: empty file is rejected by GedFileValidator (mocked to throw)
        CreateDocumentRequest request = new CreateDocumentRequest(
            "SOP-001", "Procedimento Operacional", DocumentCategory.SOP, "Criação inicial"
        );
        ...
        doThrow(new InvalidGedFileException("Arquivo não pode ser vazio."))
                .when(gedFileValidator).validate(emptyFile);
